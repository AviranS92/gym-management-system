<?php 
	...
	if($_REQUEST['message_type'])
	{
		...
		if($_REQUEST['message_type'] == 3) {
			$userlist_list = get_new_optionlist("subscriber","subscriber_id","subscriber_name",0);
			$selected_2 = "selected";
		}
	}
?>
			  ...
			  <div class="form-group" style="margin-top:10px;">
				...
				<div class="col-sm-10">
				  <select class="form-control" id="sel1" name="message_type" required onChange="submitPage(this)">
					...
					<option value="2" <?=$selected_1?>>Send to Employee</option>
					<option value="3" <?=$selected_2?>>Send to Subscriber</option>
				  </select>
				</div>
			  </div>
